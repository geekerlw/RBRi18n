...
1. Распакуйте архив `RRBi18n-v1.x.x.zip`, чтобы получить две папки: `Plugins` и `RBRi18n`
2. Перетащите обе папки с их содержимым прямо в корневой каталог игры RBR
3. Система автоматически объединит папку `Plugins`; перезапишите файлы при запросе
...
```
RBRi18n/
├── RichardBurnsRally.zh.json  # Базовая игра (китайский)
├── Weather.zh.json            # Перевод погоды (китайский)
├── Options.zh.json            # Меню настроек (китайский)
├── TuneCar.zh.json            # Настройка автомобиля (китайский)
└── ...
```
